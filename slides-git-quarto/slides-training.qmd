...
- Type `git status`
- Create a `.gitignore` file and write `*.log`
...
- Type `git checkout v1.0.0` $\rightarrow$ move to the previously created tag and type `git log`
...
- Type `git checkout 0f0e96a` to move to the first commit (find the id using `git log`)
...
- Type `git checkout main` or `git checkout master` to move at the latest commit
...
![](https://docs.github.com/assets/cb-139735/mw-1000/images/help/settings/userbar-account-settings.webp){height=600 fig-align="center"}
...
```
git remote add origin https://github.com/barriern/test-training-git.git
git branch -M master
...
- In GitHub, click on the `README.md` file.
-
