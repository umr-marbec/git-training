...
- On GitHub, add `x = 1` at the end of the `README.md` file. **Do not type `git pull` !**
...
:::{.callout-note}
These are conflicts markers. They show unresolved differences between `Local` (`HEAD`) and `Remote` (commit `70a4c10`). `Git` does't know whether to chose `x = 1` or `x = 2`. This is your job
:::

- Open the file, remove conflict markers and save.
- Commit the changes
- Type `git push`
...
![](img/funny.jpg){fig-align="center"}
...
- Type `git push` and look at your file `data.csv` file on GitHub.
