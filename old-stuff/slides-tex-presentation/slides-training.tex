...
Type \code{git\ tag\ v1.0.0} and \code{git\ log}
...
Type \code{git\ checkout\ main\ (master)} \(\rightarrow\) move at the latest
commit
...
Type \code{git\ push}. \textbf{An error occurs because changes in
\texttt{Remote} have not been pulled in \texttt{Local}!}

Type \code{git\ pull} and \code{git\ status}. `'\textbf{An error occurs! It says that a conflict has occurred on the \texttt{README.md} file}.
