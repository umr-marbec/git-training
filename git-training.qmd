...
gitGraph
    ...
    commit id: "c6dc2bc"
    commit id: "07e8835" tag: "v1.0.0"
    commit id: "-07e8835" type: REVERSE
```
...
-  First, rename your local branch with the same name as the remote branch. This is done by typing `git branch -M main`
-  Type `git push -u origin main`

It connects the *local* and *remote* main branches (`-u` option) and sends the local commits to the remote branch
...
-  Open the file, replace the above by `x = 3` and remove the conflict markers.
...
-   Now type `git clone https://github.com/umr-marbec/git-training.git`
...
- Move to the cloned folder by typing `cd git-training`
...
## Creating a repository: the easy way!

To create a new repo in a much easier way than previously discussed:

- Create a repository on GitHub with a `README.md` file and eventually a `LICENCE` file.
- Clone the repo.
    -   The `remote` and `local` repositories are automatically synchonized
    -   The `remote` and `local` main branches are automatically synchonized
- Add the files in the cloned folder
- Commit the files
- Push the files
- You are all set!
